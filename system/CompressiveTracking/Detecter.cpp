...
// Detecter初始化
void Detecter::init(const Mat& frame1, //表示视频的初始帧
			   ...
			   )
{
  ...
  iisqsum.create(frame1.rows+1,frame1.cols+1,CV_64F); //创建积分图像，创建的图像比原始图像多一行一列
  ...
  */
  ...
  //Detecter中定义 cv::Mat pEx; //positive NN example 大小为15*15图像片
  //创建P专家
  ...
  /*
	...
	//Detecter中定义：cv::PatchGenerator generator;
 ...
 */
void Detecter::generatePositiveData(const Mat& frame, int num_warps)
{
  /*
     ...
     if (i>0)
       ...
       for (int b=0;b<good_boxes.size();b++){
         ...
         classifier.getFeatures(patch,grid[idx].sidx,fern);//getFeatures函数得到
		                                                   ...
		                                                   //grid[idx].sidx 对应的
		                                                   //尺度索引 
         pX.push_back(make_pair(fern,1)); //positive ferns <features, labels=1>  正
		                                  //样本 
     }
  }
...
*/
void Detecter::getPattern(const Mat& img, Mat& pattern,Scalar& mean,Scalar& stdev){
  //Output: resized Zero-Mean patch
...
/*Detecter::generateNegativeData(const Mat& frame)
 * Inputs:
 * - Image
 ...
 */
void Detecter::generateNegativeData(const Mat& frame)
{
  /*
...
double Detecter::getVar(const BoundingBox& box,const Mat& sum,const Mat& sqsum)
{
...
void Detecter::processFrame(const cv::Mat& img1,const cv::Mat& img2,vector<Point2f>& points1,vector<Point2f>& points2,BoundingBox& bbnext,bool& lastboxfound, bool tl, FILE* bb_file){
  ...
  //Detecter只跟踪单目标，所以综合模块综合跟踪器跟踪到的单个目标和检测器检测到的多个目标
  //，然后只输出保守相似度最大的一个目标
  if (tracked){
      bbnext=tbb;
 ...
 */
void Detecter::track(const Mat& img1, const Mat& img2,vector<Point2f>& points1,vector<Point2f>& points2)
{
...
void Detecter::bbPoints(vector<cv::Point2f>& points,const BoundingBox& bb)
{
  int max_pts=10;
  int margin_h=0; //采样边界  
  int margin_v=0;
...
void Detecter::bbPredict(const vector<cv::Point2f>& points1,
...
void Detecter::detect(const cv::Mat& frame){
  //cleaning
  dbb.clear();
  dconf.clear();
  dt.bb.clear();
...
//  imshow("detections",img);
  ...
  for (int i=0;i<detections;i++){                                         //  for every remaining detection
	  ...
	  //相关相似度大于阈值，则认为含有前景目标
      if (dt.conf1[i]>nn_th){                                               //  idx = dt.conf1 > Detecter.model.thr_nn; % get all indexes that made it through the nearest neighbour
...
void Detecter::evaluate(){
}

void Detecter::learn(const Mat& img){
 // printf("[Learning] ");
...
*/
//最后将建立的窗格放在grid容器内，进行参数传递，grid是Detecter类的一个成员变量。
void Detecter::buildGrid(const cv::Mat& img, //表示帧
    ...
    for (int y=1;y<img.rows-height;y+=round(SHIFT*min_bb_side)){ //按步长移动窗口
      for (int x=1;x<img.cols-width;x+=round(SHIFT*min_bb_side)){
		...
		*/
        ...
        grid.push_back(bbox); //grid 在Detecter中定义：std::vector<BoundingBox> grid
...
*/
float Detecter::bbOverlap(const BoundingBox& box1,const BoundingBox& box2){
...
/*
 ...
 （即最相似，重叠度最大）的num_closest_init(10)个窗口，然后把这些窗口归为good_boxes容器。同时把重叠度
  小于0.2的归入到bad_boxes容器；相当于对全部的扫描窗口进行筛选。并通过BBhull函数得到这些扫描窗口的最大边界。
*/
void Detecter::getOverlappingBoxes(const cv::Rect& box1,int num_closest){
  float max_overlap = 0;
  for (int i=0;i<grid.size();i++){
      ...
      if (grid[i].overlap > 0.6){ //重叠度大于0.6的，归入good_boxes
...
int Detecter::clusterBB(const vector<BoundingBox>& dbb,vector<int>& indexes){
...
/*
	...
	之间具有更多的相似性
*/
void Detecter::clusterConf(const vector<BoundingBox>& dbb,const vector<float>& dconf,vector<BoundingBox>& cbb,vector<float>& cconf){
  int numbb =dbb.size();
  vector<int> T;
  float space_thr = 0.5;
  int c=1; //记录 聚类的类个数  
...
void Detecter::detectimg(const cv::Mat& img, cv::Rect& box,bool&_trackstu ){
	 vector<BoundingBox> cbb;
	 vector<float> cconf;
	  detect(img);
      if(detected)
	  {                           //  and detector is defined
          ...
          if (cconf.size()==1)
		  {
			  box.x=cbb[0].x;
			  box.y=cbb[0].y;
			  box.width=cbb[0].width;
			  box.height=cbb[0].height;
			  _trackstu=true;
			  cout<<"检测成功"<<endl;
			  //lastbox=cbb[0];
			  learn(img);
