...
*/
{
	...
	{
		numRect = cvFloor(rng.uniform((double)featureMinNumRect, (double)featureMaxNumRect));    //cvFloor返回不大于参数的最大整数值
		                                                                                         //rng.uniform产生一个featureMinNumRect到featureMaxNumRect间的double类型随机数，2或3
	    
		//int c = 1;
		for (int j=0; j<numRect; j++)    //在目标框中取特征区域
		{
...
void Tracker::processFrame(Mat& _frame, Rect& _objectBox,bool&_trackstu)
{
	...
	if(ctconf>ThrC)
	{
		...
		//cout<<"相关度阈值: "<<ThrC<<endl;
	    //cout<<"目标相关度: "<<ctconf<<endl;
	}
	else 
	{
		_trackstu=false;
