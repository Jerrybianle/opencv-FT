...
void Classer::init(){
  ...
  valid = 0.5;
  ncc_thesame =0.95;
  nstructs = 10; //树木（由一个特征组构建，每组
                                     //特征代表图像块的不同视图表示）的个数  
  structSize =13; //每棵树的特征个数，也即每棵树的
                                          //节点个数；树上每一个特征都作为
                                          //一个决策节点  
  thr_fern = 0.6;
  thr_nn = 0.65;
...
void Classer::prepare(const vector<Size>& scales){
  acum = 0;
  ...
  （像素比较集）的;
  pixel comparisons的产生方法：先用一个归一化的patch去离散化像素空间，
  产生所有可能的垂直和水平的pixel comparisons 然后我们把这些
  pixel comparisons随机分配给n个分类器，每个分类器得到完全不同的
...
void Classer::getFeatures(const cv::Mat& image,const int& scale_idx, vector<int>& fern){
  int leaf;//叶子  树的最终节点 
...
float Classer::measure_forest(vector<int> fern) {
  float votes = 0;
  for (int i = 0; i < nstructs; i++) {
...
//更新正负样本数，同时更新后验概率  
void Classer::update(const vector<int>& fern, int C, int N) {
  int idx;
  for (int i = 0; i < nstructs; i++) {
      idx = fern[i];
...
//训练集合分类器（n个基本分类器集合） 
void Classer::trainF(const vector<std::pair<vector<int>,int> >& ferns,int resample){
...
//训练最近邻分类器
void Classer::trainNN(const vector<cv::Mat>& nn_examples){
  float conf,dummy;
...
void Classer::NNConf(const Mat& example, vector<int>& isin,float& rsconf,float& csconf){
  /*Inputs:
   * -NN Patch
   * Outputs:
...
void Classer::evaluateTh(const vector<pair<vector<int>,int> >& nXT,const vector<cv::Mat>& nExT){
float fconf;
